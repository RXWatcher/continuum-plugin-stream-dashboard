...
import (
	...
	"github.com/RXWatcher/silo-plugin-stream-dashboard/internal/poll"
	pluginrt "github.com/RXWatcher/silo-plugin-stream-dashboard/internal/runtime"
	"github.com/RXWatcher/silo-plugin-stream-dashboard/internal/server"
	"github.com/RXWatcher/silo-plugin-stream-dashboard/internal/store"
	"github.com/RXWatcher/silo-plugin-stream-dashboard/web"
)
...
func main() {
	logger := hclog.New(&hclog.LoggerOptions{Name: "silo-plugin-stream-dashboard"})
	...
	rt := pluginrt.New(manifest, func(cfg pluginrt.Config) error {
		...
		if err != nil {
			pool.Close()
			return fmt.Errorf("connect silo source database: %w", err)
		}
		...
		if err := sourcePool.Ping(ctx); err != nil {
			pool.Close()
			sourcePool.Close()
			return fmt.Errorf("ping silo source database: %w", err)
		}
